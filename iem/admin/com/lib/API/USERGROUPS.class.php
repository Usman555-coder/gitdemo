...
class API_USERGROUPS implements IEM_baseAPI
{
	public static function getRecords($countonly = false, $condition = false, $limit = 0, $offset = 0, $sortby = false, $sortdesc = false)
	{
		...
		if ($countonly) {
			...
			if (!empty($sortby) && $sortby !== 'usercount') {
        ...
        if ($sortby == 'usercount') {
            ...
            foreach ($records as $key => $row) {
                $count[$key] = $row['usercount'];
            }

            array_multisort($count, ($sortdesc? SORT_ASC : SORT_DESC), $records);
        }
