...
<script>
    ...
    function ChangeFormType(selectedVal) {
        ...
        if (elementToHide.length != 0)
            $(elementToHide.join(', ')).hide();
        if (elementToShow.length != 0)
            $(elementToShow.join(', ')).show();
        ...
        for (var i = 0, j = tempFieldOrder.size(); i < j; ++i) {
            ...
            if (jQuery.inArray(tempFieldOrder.get(i).value, allowableValues) != -1)
                tempRemove = false;
            if (tempFieldOrder.get(i).value == 'cl' && tempListCount <= 1)
                tempRemove = true;

            if (tempRemove)
                RemoveFieldOrderValue(tempFieldOrder.get(i));
        }


        if (checkFormat && $('#SubscriberChooseFormat').val() == 'c')
            CheckFormat();
    }
    ...
    function CheckFormat() {
        ...
        if (!foundFomat)
            AddFieldOrderValue(RequiredFields_Values['cf'], 'cf');
    }
