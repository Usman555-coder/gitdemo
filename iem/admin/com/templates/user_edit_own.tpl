...
<script>
    $(function () {
        ...
        $('.CancelButton', document.settings).click(function () {
            if (confirm('Are you sure you want to cancel?'))
                document.location.href = 'index.php?Page=ManageAccount';
        });

        $('#usewysiwyg').click(function () {
            $('#sectionUseXHTML').toggle();
        });

        $(document.settings.smtptype).click(function () {
            $('.SMTPDetails')[document.settings.smtptype[1].checked ? 'show' : 'hide']();
            if (document.settings.smtptype[2])
                $('.sectionSignuptoSMTP')[document.settings.smtptype[2].checked ? 'show' : 'hide']();
        });
