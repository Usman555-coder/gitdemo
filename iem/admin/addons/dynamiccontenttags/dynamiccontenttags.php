...
class Addons_dynamiccontenttags extends Interspire_Addons {
    public $_cacheCustomFieldsUsedByLists = [];
    private $tags = [];
    private $sortDetails = [];
    private $perPage = 0;
    private $start = 0;
    ...
    public static function SetMenuItems(EventData_IEM_SENDSTUDIOFUNCTIONS_GENERATEMENULINKS $data) {
        $self = new self ( );
        $news_dynamic_content_tag_menu = ['text' => GetLang ( 'Addon_dynamiccontenttags_Menu_DynamicContentTags' ), 'link' => $self->admin_url, 'image' => '../addons/dynamiccontenttags/images/dct_add.gif', 'show' => ['CheckAccess' => 'HasAccess', 'Permissions' => ['dynamiccontenttags']], 'description' => GetLang ( 'Addon_dynamiccontenttags_Menu_DynamicContentTags_Description' )];
    ...
    public static function RegisterAddonPermissions() {
        $perms = [
            ...
            'general' => ['name' => GetLang('Addon_dynamiccontenttags_Permission_General')],
            ],
        ];
                            ...
                            if ($continue) {
                                ...
                                switch ($ruleEntry->rules->ruleType) {
                                    ...
                                    case 'checkbox':
                                        ...
                                        $tempRuleValues = explode(', ', $tempRuleValues);
                                        $tempRuleValues = (is_array($tempRuleValues)) ? $tempRuleValues : [];
                                        $tempActualValues = (is_array($tempActualValues)) ? $tempActualValues : [];
                                        switch ($ruleEntry->rules->ruleOperator) {
                                            case 'equalto':
    ...
    public function Admin_Action_ShowDynamicContentTag() {
        ...
        while ( $row = $this->db->Fetch ( $result ) ) {
            $tmpTags [] = [$row ['tagid'], $row ['name'], $row ['createdate']];
        }
        ...
        if (sizeof ( $tmpTags )) {
            ...
            $this->template_system->Assign ( 'CloseButton', '<input type="button" class="FormButton CancelButton btn cancel-btn" value="'.GetLang('Close').'" onclick="linkWin.close(); return false;"/>' );
            $this->template_system->ParseTemplate ( 'DynamicContentTags_List_Start' );
        }
    ...
    public function Admin_Action_UpdateBlocksOrder() {
        $blockIds = $this->_getPOSTRequest ( 'blockid', []);
    ...
    public function Admin_Action_DeleteBlock() {
        ...
        if ($blockId) {
            ...
            // Error message
                ...
                echo GetJSON ( ['message' => GetFlashMessages (), 'result' => '1']);
                return;
            }
        }
        FlashMessage ( GetLang ( 'Addon_dynamiccontenttags_DeleteBlock_Failure' ), SS_FLASH_MSG_ERROR );
        echo GetJSON ( ['message' => GetFlashMessages (), 'result' => '0']);
    }
     ...
     */
    public function _getCustomFieldUsedByList($listIDs) {
            ...
            foreach ( $listIDs as $tempID ) {
                ...
                foreach ( $tempStatus as $tempEach ) {
                    ...
                    if (! array_key_exists ( $tempEach ['fieldid'], $tempOutput ['customfields'] )) {
                        ...
                        $tempOutput ['customfields'] [$tempEach ['fieldid']] = ['name' => htmlspecialchars ( $tempEach ['name'], ENT_QUOTES, SENDSTUDIO_CHARSET ), 'fieldtype' => $tempEach ['fieldtype'], 'defaultvalue' => $tempEach ['defaultvalue'], 'operatortype' => $tempFieldType];
                    }
                    /**
                     * -----
                     */
                    ...
                    if (! array_key_exists ( $tempEach ['fieldid'], $tempOutput ['values'] )) {
                        ...
                        if (is_array ( $temp ) && array_key_exists ( 'Key', $temp ) && array_key_exists ( 'Value', $temp )) {
                            foreach ( $temp ['Key'] as $index => $value ) {
                                array_push ( $tempFieldValues, ['value' => $value, 'text' => htmlspecialchars ( $temp ['Value'] [$index], ENT_QUOTES, SENDSTUDIO_CHARSET )]);
                            }
                        }
    ...
    public function Admin_Action_Save() {
        ...
        // Tag lists
        $lists = $this->_getPOSTRequest ( 'SelectList', []);

        // Content Blocks Properties
        $tmpBlocks = $this->_getPOSTRequest ( 'blocks', []);
    ...
    public function loadTags($id = 0) {
        ...
        $result = $this->db->Query ( $query );
        while ( $row = $this->db->Fetch ( $result ) ) {
            $tmpTags [] = new DynamicContentTag_Api_Tag ( $row ['tagid'], $row ['name'], $row ['createdate'], $row ['ownerid'], [], []);
        }
     ...
     *
     */
    public function loadTagsByList($listId = []) {
        $tmpTags = [];
