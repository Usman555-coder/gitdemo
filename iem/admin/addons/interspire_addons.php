...
class Interspire_Addons
{
	...
	final function Process()
	{
		...
		$func = preg_replace('/[^\w]/', '_', strtolower($args[1]));
	...
	protected function SetupPaging($url=null, $number_of_records=0, $number_of_links_to_show=5)
	{
		...
		$pageofpage = 'Showing ' . $this->FormatNumber($current_page) . ' ' . GetLang('Of') . ' ' . $this->FormatNumber($num_pages) . ' Pages';
		$GLOBALS['pageofpage'] = $pageofpage;
		$display_page_name = 'DisplayPage';

		if ($current_page > 1) {
			$string .= '<a class="page-link" href="' . $url . '&' . $display_page_name . '=1" title="' . GetLang('GoToFirst') . '">&laquo;</a>';
			$string .= '<a class="page-link" href="' . $url . '&' . $display_page_name . '=' . $prevpage . '">' . GetLang('PagingBack') . '</a>';
		} else {
		...
		for ($pageid = $start_page; $pageid <= $end_page; $pageid++) {
			...
			if ($pageid == $current_page) {
				...
				$string .= '<a class="page-link" href="' . $url . '&' . $display_page_name . '=' . $pageid . '">' . $pageid . '</a>';
			}
			$string .= '';
		}
		...
		if ($current_page == $num_pages) {
			...
			$string .= '<a class="page-link" href="' . $url . '&' . $display_page_name . '=' . $nextpage . '">' . GetLang('PagingNext') . '</a>';
			$string .= '<a class="page-link" href="' . $url . '&' . $display_page_name . '=' . $num_pages . '" title="' . GetLang('GoToLast') . '">&raquo;</a>';
		}
