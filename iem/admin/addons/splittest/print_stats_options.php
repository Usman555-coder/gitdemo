...
class PrintStats extends Addons_splittest
{
	...
	{
        (new SendStudio_Functions())->LoadLanguageFile('stats');
		...
		if ($statstype == null) {
            ...
            $this->template_system->Assign('print_options', '');
            $this->template_system->ParseTemplate('print_stats_options');
            return;
		}

		$this->GetApi('Splittest_Stats');
		...
		switch ($statstype) {
			case 'splittest' :
				...
				foreach ($options as $key => $val) {
					...
					$print_options .= '<div class="printDiv" style="background-color: ' . $bg_color . '; padding: 5px; margin-bottom: 5px;">';
					$print_options .= '<label class="customCheckBox">
					<input id="print_' . $key . '" type="checkbox" name="options[]" value="' . $key . '" checked="checked" class="UICheckboxToggleSelector" />
