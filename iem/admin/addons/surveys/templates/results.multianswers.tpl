	...
	<table class="survey_results">
		...
		{foreach from=$stats key=answer item=stat}
			{if $maxstats == $stat && $maxstats != 0}
				...
				<td class="tablewidth"><div class="bar"><div class="survey_percentage" style="width: {$percentage.$answer}%;"></div></div></td>
		...
		{if $total_others > 0 }	
			...
			{else}
				<tr>
			{/if}
					<td align="right"><a class="others_hide" id="more_{$question_id}">Hide Answers</a> ({$other_label})</td><td class="tablewidth"><div class="bar"><div class="survey_percentage" style="width: {$percentage.others}%;"></div></div></td>
			...
			<tbody id="othersmore_{$question_id}">
				{foreach from=$other_answers key=index item=other_answer}
					<tr>
						<td></td><td class="tablewidth">{$index}. {$other_answer.value}</td><td class="browse"><a href="index.php?Page=Addons&Addon=surveys&Action=viewresponses&surveyId={$surveyId}&responseId={$other_answer.id}">Browse...</a></td><td></td>
					</tr>
				{/foreach}
					<tr>
