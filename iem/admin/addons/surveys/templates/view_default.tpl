...
<style>
    ...
    table.table-selector td:nth-child(1) {
        ...
        padding-rpafight:5px !important;
    }
...
<h1 class="PageTitle Heading1 Heading1New">
{$lang.Addon_Surveys_viewResponsesTitle}</h1>
<div class="PageDescription" style="padding:5px 0px 20px 0px">
{$lang.Addon_Surveys_viewResponsesDescription}</div>
			...
			<ul>
							...
							<table class="table-selector" style="width: 100%">
								<tbody>
									{foreach from=$forms key=k item=form}
										<tr>
											<td align="middle">
											<label class="customRadio"><input {if $form.responseCount == 0}class="no-responses"{/if} type="radio" name="surveyId" value="{$form.id}" />
				...
				<li>
					<div class="formButtons">
						<button class="next btn save-btn SubmitButton" type="submit">{$lang.Addon_Surveys_viewResponsesButtonNext}</button>
						<button class="cancel btn cancel-btn cancel" type="button">{$lang.Addon_Surveys_viewResponsesButtonCancel}</button>
					</div>
				</li>
			</ul>
	</form>
{else}
    ...
    color:#000000;'>
		...
		<tr>
			<td class="body"><input type="submit" name="Action" value="%%LNG_Addon_surveys_Create%%" class=" createSurveyButton btn save-btn" style="width:16rem" onclick="location.href='index.php?Page=Addons&Addon=surveys&Action=Create';return false;"></td>
		</tr>
	</table>
{/if}
	<style>
