	...
	public function Admin_Action_Default()
	{
		...
		foreach ($surveys as $survey) {
			...
			if (empty($survey['responseCount'])) {
				$view_results = '<span class="dropdown-item disabled-item">'.GetLang('Addon_Surveys_Default_Table_ViewResults').'</span>';
				$export_responses = '<span class="dropdown-item disabled-item">'.GetLang('Addon_Surveys_Default_Table_ExportResponses').'</span>';

			} else {
				$view_results = '<a  class="dropdown-item" href="' . $me->admin_url . '&Action=result&surveyId=' . $survey['id'] . '">' . GetLang('Addon_Surveys_Default_Table_ViewResults') . '</a>';
				$export_responses = '<a  class="dropdown-item" href="' . $me->admin_url . '&Action=Export&ajax=1&surveyId=' . $survey['id'] . '">' . GetLang('Addon_Surveys_Default_Table_ExportResponses') . '</a>';
			}
			...
			$editlink = '<a  class="dropdown-item" href="' . $me->admin_url . '&Action=Edit&formId=' . $survey['id'] . '">' . GetLang('Edit') . '</a>';
			$me->template_system->Assign('edit_link',$editlink,false);

			$deletelink = '<a  class="dropdown-item" href="' . $me->admin_url . '&Action=Delete&id=' . $survey['id'] . '">' . GetLang('Delete') . '</a>';
			$me->template_system->Assign('delete_link',$deletelink,false);

			$previewlink = '<a  class="dropdown-item" target="_blank" href="' . SENDSTUDIO_APPLICATION_URL . '/surveys.php?id=' . $survey['id'] . '">' . GetLang('Preview') . '</a>';
