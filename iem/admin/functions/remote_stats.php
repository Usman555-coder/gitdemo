...
class RemoteStats extends SendStudio_Functions
{
						...
						switch ($statstype) {
							...
							default:
								...
								foreach ($a as $key => $val) {
									...
									$print_options .= '<div class="printDiv" style="background: ' . $bg_color . '; padding: 10px; margin-bottom: 5px;">';
									$print_options .= '<label class="customCheckBox"><input id="print_'.$key.'" type="checkbox" name="options[]" value="'.$key.'" checked="checked" style="margin:0;" class="UICheckboxToggleSelector" /><span class="checkmark"></span></label>
										<label for="print_' . $key . '">'.$val.'</label><br /><br />' . "\n";
									...
									if ($key != 'snapshot') {
										...
										$print_options .= '<span style="width: 20px;"><img src="images/blank.gif" width="24" height="1" /></span>';
										$print_options .= '<label class="customCheckBox parentChildLabel"><input class="UICheckboxToggleSelector" id="print_details_'.$key.'" type="checkbox" name="options_details[]" value="'.$key.'" style="margin:0;"';
										if ($count == 0) {
											$print_options .= ' disabled="disabled"';
											$print_options .= ' /><span class="checkmark disabledCheckbox"></span></label>
												<label class="printChildLabel" for="print_details_' . $key . '">' .
												sprintf(GetLang('Stats_Print_IncludeDetailsOf'),$count,GetLang("Stats_Print_$key")) .
												'</label><br />' . "\n";
										} else {
											$print_options .= ' /><span class="checkmark"></span></label>
											<label class="printChildLabel" for="print_details_' . $key . '">' .
											sprintf(GetLang('Stats_Print_IncludeDetailsOf'),$count,GetLang("Stats_Print_$key")) .
											'</label><br />' . "\n";
										}
	...
	*/
	...
	{ 
		...
		if ($currentpage > 1) {
			$string .= "<a class=\"page-link\" href=\"javascript:REMOTE_admin_table($('#adminTable{$type}'),'$url','','$type','$token',1,'{$GLOBALS['SortColumn']}','{$GLOBALS['SortDirection']}')\" title=\"" . GetLang('GoToFirst') . "\">&laquo;</a>";
			$string .= "<a class=\"page-link\" href=\"javascript:REMOTE_admin_table($('#adminTable{$type}'),'$url','','$type','$token',$prevpage,'{$GLOBALS['SortColumn']}','{$GLOBALS['SortDirection']}')\" title=\"" . GetLang('PagingBack') . "\">".GetLang('PagingBack') ."</a>";
		} else {
		...
		for ($pageid = $start_page; $pageid <= $end_page; $pageid++) {
			...
			if ($pageid == $currentpage) {
				...
				$string .= "<a class=\"page-link\" href=\"javascript:REMOTE_admin_table($('#adminTable{$type}'),'$url','','$type','$token',$pageid,'{$GLOBALS['SortColumn']}','{$GLOBALS['SortDirection']}')\">$pageid</a>";
			}
			$string .= '';
		}

		if ($currentpage == $num_pages) {
			$string .= '<a class="page-link page-item">' . GetLang('PagingNext') . '</a>';
			$string .= '<a class="page-link page-item">&raquo;</a>';
		} else {
			$string .= "<a class=\"page-link\" href=\"javascript:REMOTE_admin_table($('#adminTable{$type}'),'$url','','$type','$token',$nextpage,'{$GLOBALS['SortColumn']}','{$GLOBALS['SortDirection']}')\">" . GetLang('PagingNext') . "</a>";
			$string .= "<a class=\"page-link\" href=\"javascript:REMOTE_admin_table($('#adminTable{$type}'),'$url','','$type','$token',$num_pages,'{$GLOBALS['SortColumn']}','{$GLOBALS['SortDirection']}')\" title=\"" . GetLang('GoToLast') . "\">&raquo;</a>";
		}
