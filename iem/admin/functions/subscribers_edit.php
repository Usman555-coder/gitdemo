	...
	function EditSubscriber($listid = 0, $subscriberid = 0, $segmentid = 0, $msgtype = 'Error', $msg = false)
	{
		...
		if (!empty($customfields)) {
			...
			foreach ($customfields as $pos => $customfield_info) {
				...
				if($GLOBALS['Required']=="&nbsp;&nbsp;") { $GLOBALS['Required']=''; }
				...
				switch ($customfield_info['fieldtype']) {
					...
					case 'checkbox':
						...
						foreach ($fieldsettings['Key'] as $pos => $key) {
							...
							$optionlist .= '<label class="customCheckBox" style="margin-right: 5px;"  for="' . $label . '"><input type="checkbox" style="margin-right: 4px; "  name="CustomFields[' . $customfield_info['fieldid'] . '][' . $pos . ']" id="' . $label . '" value="' . htmlspecialchars($key, ENT_QUOTES, SENDSTUDIO_CHARSET) . '"' . $selected . '>' . htmlspecialchars($fieldsettings['Value'][$pos], ENT_QUOTES, SENDSTUDIO_CHARSET) . '<span class="checkmark"></span></label>';
							if ($c % 4 == 0) {
								$optionlist .= '<br/>';
							}
