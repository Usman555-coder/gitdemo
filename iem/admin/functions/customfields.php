	...
	*/
	function CreateCustomField_Step1()
	{
		...
		foreach ($this->CustomFieldTypes as $p => $type) {
			$typelist .= '<label for="' . $type . '" class="customRadio"><input onclick="cf_selected=true;" type="radio" name="FieldType" id="' . $type . '" value="' . $type . '">' . GetLang('CustomFieldType_' . strtolower($type)) . '<span class="checkmarkRadio"></span></label> - <span style="color:#AAA" class="customFieldSpan">' . GetLang('CustomFieldDesc_' . strtolower($type)) . '</span>';
	...
	function ManageCustomFields()
	{
		...
		foreach ($myfields as $pos => $fieldinfo) {
			...
			if ($user->Admin() || ($user->HasAccess('customfields', 'edit') && $user->Get('userid') == $api->Get('ownerid'))) {
				$GLOBALS['CustomFieldAction'] .= '<a class="dropdown-item" href="index.php?Page=CustomFields&Action=Edit&id=' . $fieldinfo['fieldid'] . '">' . GetLang('Edit') . '</a>';
			} else {
			...
			if ($user->Admin() || ($user->HasAccess('customfields', 'delete') && $user->Get('userid') == $api->Get('ownerid'))) {
				$GLOBALS['CustomFieldAction'] .= '<a class="dropdown-item" href="javascript: ConfirmDelete(' . $fieldinfo['fieldid'] . ');">' . GetLang('Delete') . '</a>';
			} else {
