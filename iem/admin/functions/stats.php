...
class Stats extends SendStudio_Functions
{
	...
	*/
	function PrintNewsletterStats_Step1()
	{
		...
		foreach ($mystats as $pos => $statsdetails) {
			...
			$GLOBALS['StatsAction'] = '<a class="dropdown-item" href="index.php?Page=Stats&Action=Newsletters&SubAction=ViewSummary&id=' . $statsdetails['statid'] . '">' . GetLang('ViewSummary') . '</a>';
			$GLOBALS['StatsAction'] .= '<a class="dropdown-item"  href="index.php?Page=Stats&Action=Newsletters&SubAction=DoSelect&SelectAction=export&stats%5B%5D=' . $statsdetails['statid'] . '">' . GetLang('Export_Stats_Selected') . '</a>';
			$GLOBALS['StatsAction'] .= '<a href="remote_stats.php?height=420&width=420&overflow=none&statstype=n&Action=print&stats%5B%5D=' . $statsdetails['statid'] . '" class="thickbox dropdown-item" title="' . GetLang('PrintEmailCampaignStatistics') . '">' . GetLang('Print_Stats_Selected') . '</a>';

			$finishtime = $statsdetails['finishtime'];
			if ($finishtime > 0) {
				$GLOBALS['StatsAction'] .= '<a class="dropdown-item" href="javascript:ConfirmDelete(' . $statsdetails['statid'] . ');">' . GetLang('Delete') . '</a>';
	...
	*/
	...
	{
        include_once (SENDSTUDIO_FUNCTION_DIRECTORY . '/lib/amcharts/amcharts.php');
	...
	*/
	function PrintUserStats_Step2($userid=0)
	{
		...
		$GLOBALS['SummaryChart'] = AmCharts::InsertPieChart($data_url, GetLang("User_Summary_Graph"));
	...
	function GetCalendarInfo($calendar=array(), $dateonly=false)
	{
		...
		switch ($calendar_settings['DateType']) {
			...
			case 'Last7Days':
				...
				$datetoshow = $this->PrintDate(AdjustTime($seven_daysago), $date_format);
				$datetoshow .= ' - ' . $this->PrintDate($timenow, $date_format);
			break;
			...
			case 'ThisMonth':
				...
				$datetoshow = $this->PrintDate(AdjustTime($startofmonth), $date_format);
				$datetoshow .= ' - ' . $this->PrintDate($timenow, $date_format);
			break;
			...
			case 'Custom':
				...
				$datetoshow = $this->PrintDate(AdjustTime($start), $date_format) . ' - ' . $this->PrintDate(AdjustTime($end), $date_format);
			break;

			case 'LastMonth':
				$lastmonth = mktime(0, 0, 0, date('m')-1, 1, date('Y'));
				$thismonth = mktime(0, 0, 0, date('m'), 1, date('Y'));
				$datetoshow = $this->PrintDate(AdjustTime($lastmonth), $date_format);
				$datetoshow .= ' - ' . $this->PrintDate(AdjustTime($thismonth), $date_format);
			break;
	...
	*/
	function DisplayChart($chartname='', $chart_area='', $statid=0, $type = 'pie', $settings = null, $listid=0)
	{
        ...
        $title = (!is_array($settings) || empty($settings['graph_title'])) ? '' : $settings['graph_title'];

        if ($type === 'pie') {
            $GLOBALS[$chartname] = AmCharts::InsertPieChart($dataUrl, $title);
        }

        if ($type === 'column') {
            $GLOBALS[$chartname] = AmCharts::InsertColumnChart($dataUrl, $title);
        }
	...
	*/
	function PrintAutoresponderStats_Step1($msg='')
	{
		...
		foreach ($mystats as $pos => $statsdetails) {
			...
			$GLOBALS['StatsAction'] = '<a class="dropdown-item" href="index.php?Page=Stats&Action=Autoresponders&SubAction=ViewSummary&auto=' . $statsdetails['autoresponderid'] . '">' . GetLang('ViewSummary') . '</a>';

                        $GLOBALS['StatsAction'] .= '<a href="remote_stats.php?height=420&width=420&overflow=none&statstype=a&Action=print&stats%5B%5D=' . $statsdetails['autoresponderid'] . '" class="thickbox dropdown-item"title="' . GetLang('PrintAutoresponderStatistics') . '">' . GetLang('Print_Stats_Selected') . '</a>';
	...
	*/
	function PrintListStats_Step1()
	{
		...
		foreach ($mylists as $pos => $listdetails) {
			...
			$GLOBALS['StatsAction'] = '<a class="dropdown-item" href="index.php?Page=Stats&Action=List&SubAction=ViewSummary&list=' . $listdetails['listid'] . '">' . GetLang('ViewSummary') . '</a>';

			$GLOBALS['StatsAction'] .= '<a href="remote_stats.php?height=475&width=420&overflow=none&statstype=l&Action=print&stats%5B%5D=' . $listdetails['listid'] . '" class="thickbox dropdown-item" title="' . GetLang('PrintListStatistics') . '">' . GetLang('Print_Stats_Selected') . '</a>';

			if ($user->isAdmin()) {
				$GLOBALS['StatsAction'] .= '<a class="dropdown-item" href="index.php?Page=Users&Action=Edit&UserID=' . $listdetails['ownerid'] . '" target="_blank">' . GetLang('EditListOwner') . '</a>';
			}
	...
	// --------------------------------------------------------------------------------
		...
		function TriggerEmailsStats_View()
		{
			...
			// ----- Tab 1: Snapshot
				...
				$tabs['snapshot']['summary_chart'] = AmCharts::InsertPieChart(
					...
					sprintf(GetLang('TriggerEmails_Stats_Snapshots_ChartTitle'), $record['triggeremailsname']));
			// -----
