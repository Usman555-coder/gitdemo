...
class SendStudio_Functions {
    ...
    function PrintHeader($PopupWindow = false, $LoadLanguageFile = true, $PrintHeader = true) {
        ...
        if ($PrintHeader) {
            ...
            //var_dump($queryArray); exit;

            $top_menu = array(
                    'Settings', 'settings', 'templates', 'Templates', 'Forms', 'forms', 'users', 'Users', 'UsersGroups', 'usersgroups', 'dbcheck', 'systemlog', 'checkpermissions', 'ManageAccount', 'manageaccount', 'debug', 'Debug'
                );
            $topMenu = array_intersect($top_menu, $queryArray);
            if (count($topMenu) > 0) {
                $tpl->ParseTemplate('header_old');
            } else {
    ...
    function InfotipNew($PopupWindow = false, $LoadLanguageFile = true, $PrintHeader = true) {
        $user = IEM::getCurrentUser();
        ...
        //     $top_menu = array(
        //             'Settings', 'Templates', 'Forms', 'Users', 'UsersGroups', 'dbcheck', 'systemlog', 'checkpermissions', 
        //         );
        //     $topMenu = array_intersect($top_menu, $queryArray);
        //     if (count($topMenu) > 0) {
        //         $tpl->ParseTemplate('header_old');
    ...
    static function GenerateTextMenuLinks() {
        ...
        $setting_menu_items = array();
        $setting_menu_list = $newvar;
        unset($setting_menu_list['logout']);
        unset($setting_menu_list['home']);
        //unset($setting_menu_list['templates']);
        //unset($setting_menu_list['forms']);
        //echo "<pre>"; print_r($setting_menu_list); exit;
        foreach ($setting_menu_list as $link_area => $sublinks) {
            $setting_menu_list[$link_area]=0;
            $has_submenu = false;
            ...
            if (!$has_submenu) {
                ...
                //echo "<pre>"; print_r($link_details); exit;
                if (isset($link_details['show']) && $link_details['show'] == true) {
                    $setting_menu_list[$link_area]=$setting_menu_list[$link_area]+1;
                }
                continue;
            }
            unset($sublinks['menudetails']);
            //echo "<pre>"; print_r($sublinks); exit;
        ...
        foreach ($setting_menu_list as $i => $v) {
            if ($v == 0) {
                unset($setting_menu_list[$i]);
            }
        }
        //echo "<pre>"; print_r($setting_menu_list); exit;
        ...
        foreach ($newvar as $link_area => $sublinks) {
            $has_submenu = false;
            ...
            $sublink = '<a href="#" title="' . htmlspecialchars($newvar[$link_area]['menudetails']['description'], ENT_QUOTES, SENDSTUDIO_CHARSET) . '" id="' . $id . 'MenuButton" class="PopDownMenu MenuText">' . htmlspecialchars($newvar[$link_area]['menudetails']['title'], ENT_QUOTES, SENDSTUDIO_CHARSET) . '<img src="images/arrow_down_white.gif" border="0" style="position: relative;" /></a><div id="' . $id . 'Menu" style="display: none; width: ' . $width . 'px;" class="DropDownMenu DropShadow">' . '<ul>';
        ...
        $textlinks .= '&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&nbsp;';
        

        $GLOBALS['parent_setting_menu']=$setting_menu_list;
        return $textlinks;
    }
...
//    }
     ...
     */
    static function GenerateMenuLinksPagewise() {
        $user = IEM::getCurrentUser();
        ...
        //var_dump($menuItems['contactlist_button']); exit;
        ...
        $contactlist_button = array(
                'Lists', 'CustomFields', 'Bounce', 'Segment',
            );
        $contact_button = array(
                'Subscribers',
            );
        $newsletter_button = array(
                'Newsletters', 'Send', 'ImageManager', 'Schedule', 'dynamiccontenttags', 'splittest',
            );
        $surveys_button = array(
                'surveys',
            );
        $autoresponder_button = array(
                'Autoresponders', 'TriggerEmails',
            );
        $statistics_button = array(
                'Stats',
            );
        ...
        $cl = array_uintersect($contactlist_button, $queryArray, "strcasecmp");
        if (count($cl) > 0) {
            $menuKey = 'contactlist_button';
        }
        $c = array_uintersect($contact_button, $queryArray, "strcasecmp");
        if (count($c) > 0) {
            $menuKey = 'contact_button';
        }
        $n = array_uintersect($newsletter_button, $queryArray, "strcasecmp");
        if (count($n) > 0) {
            $menuKey = 'newsletter_button';
        }
        $s = array_uintersect($surveys_button, $queryArray, "strcasecmp");
        if (count($s) > 0) {
            $menuKey = 'surveys_button';
        }
        $a = array_uintersect($autoresponder_button, $queryArray, "strcasecmp");
        // echo "<pre>";
        // print_r($autoresponder_button);
        // var_dump($queryArray);
        // print_r($a);
        // exit;
        if (count($a) > 0) {
            $menuKey = 'autoresponder_button';
        }
        $st = array_uintersect($statistics_button, $queryArray, "strcasecmp");
        if (count($st) > 0) {
            $menuKey = 'statistics_button';
        }
        
        //echo "<pre>"; print_r($menuKey); exit;
            ...
            if (is_array($link)) {
                ...
                foreach ($link as $id => $sub) {
                    ...
                    if (!$show) {
                        ...
                        //$main_menu_items[$image] = $image;
                        $shown = true;
                        $main_menu_items[$image] = $main_menu_items[$image]+1;
                    }
                }
            }
        }
          
        foreach ($main_menu_items as $i => $v) {
            if ($v == 0) {
                unset($main_menu_items[$i]);
            }
        }
        $main_menu_items['overview_button']='1';
        //echo "<pre>"; print_r($main_menu_items); exit;

        $menu_exists = "";
        foreach ($newvar as $image => $link) {
            ...
            if (is_array($link)) {
                ...
                foreach ($link as $id => $sub) {
                    ...
                    if (count($link) > 1) {
                        ...
                        //$menu .= '<li><a href="' . $sub['link'] . '" ' . $target . '><strong>' . $sub['text'] . '</strong><span>' . $sub['description'] . '</span></a></li>' . "\n";
                        
                        //echo $_SERVER['REQUEST_URI']; 
                        //echo strstr($_SERVER['REQUEST_URI'], 'index.php?') . "<br>";
                        //echo $sub['link']; exit;
                        
                        $active="";
                        $current_link = strtolower(strstr($_SERVER['REQUEST_URI'], 'index.php?'));
                        $actual_link = strstr(strtolower(htmlspecialchars_decode($sub['link'])), 'index.php?');
                        //echo "<br>".$current_link."====".$actual_link."<br>";
                        if($current_link==$actual_link) {
                            $active=" active";
                        }
                        $menu_exists = '1';
                        $menu .= '<li class="nav-item" role="presentation"><a  class="nav-link ' . $active . '" id="permnent-tab" role="tab" aria-controls="permnent" aria-selected="true" href="' . $sub['link'] . '">' . $sub['text'] . '</a></li>';
                    }
                }
            }
        }
        $menu .= '</ul>' . "\n";
        $GLOBALS['parent_menu']=$main_menu_items;
        $GLOBALS['menu_exists']=$menu_exists;
    ...
    function ShowInfoTip($ret='') {
        ...
        if($ret=='') {
            ...
            return $this->ParseTemplate('InfoTips', true);
        }
    }
    ...
    function SetupPaging($numrecords = 0, $currentpage = 1, $perpage = 20) {
        ...
        $pageofpage = 'Showing ' . $this->FormatNumber($currentpage) . ' ' . GetLang('Of') . ' ' . $this->FormatNumber($num_pages) . ' Pages';
        $GLOBALS['pageofpage'] = $pageofpage;
        ...
        if ($currentpage > 1) {
            $string .= '<a class="page-link" href="index.php?Page=' . $page_type . $sortdetails . '&' . $display_page_name . '=1" title="' . GetLang('GoToFirst') . '">&laquo;</a>';
            $string .= '<a class="page-link" href="index.php?Page=' . $page_type . $sortdetails . '&' . $display_page_name . '=' . $prevpage . '">' . GetLang('PagingBack') . '</a>';
        } else {
            $string .= '<a class="page-link page-item">&laquo;</a>';
            $string .= '<a class="page-link page-item">' . GetLang('PagingBack') . '</a>';
        }
        ...
        for ($pageid = $start_page; $pageid <= $end_page; $pageid++) {
            ...
            if ($pageid == $currentpage) {
                $string .= '<b class="page-link page-item active">' . $pageid . '</b>';
            } else {
                $string .= '<a class="page-link" href="index.php?Page=' . $page_type . $sortdetails . '&' . $display_page_name . '=' . $pageid . '">' . $pageid . '</a>';
            }
            $string .= '';
        }

        if ($currentpage == $num_pages) {
            $string .= '<a class="page-link">' . GetLang('PagingNext') . '</a>';
            $string .= '<a class="page-link">&raquo;</a>';
        } else {
            $string .= '<a class="page-link" href="index.php?Page=' . $page_type . $sortdetails . '&' . $display_page_name . '=' . $nextpage . '">' . GetLang('PagingNext') . '</a>';
            $string .= '<a class="page-link" href="index.php?Page=' . $page_type . $sortdetails . '&' . $display_page_name . '=' . $num_pages . '" title="' . GetLang('GoToLast') . '">&raquo;</a>';
        }
    ...
    function MoveFiles($destination = false, $id = 0) {
        ...
        if (CopyDirectory($tempdir, $destinationdir)) {
                                ...
                                function Display_CustomField($customfield_info = array(), $defaults = array()) {
                                    switch ($customfield_info['fieldtype']) {
                                        case 'date':
                                            ...
                                            $daylist = '<select name="CustomFields[' . $GLOBALS['FieldID'] . '][dd]" id="CustomFields[' . $GLOBALS['FieldID'] . '][dd]" class="datefield form-control CustomField_Date_Day">';
                                            ...
                                            $monthlist = '<select name="CustomFields[' . $GLOBALS['FieldID'] . '][mm]" id="CustomFields[' . $GLOBALS['FieldID'] . '][mm]" class="datefield form-control CustomField_Date_Month">';
                                            ...
                                            $yearlist = '<select name="CustomFields[' . $GLOBALS['FieldID'] . '][yy]" id="CustomFields[' . $GLOBALS['FieldID'] . '][yy]" class="datefield form-control CustomField_Date_Year">';
                                ...
                                function Search_Display_CustomField($customfield_info = array()) {
                                    ...
                                    switch (strtolower($customfield_info['fieldtype'])) {
                                        case 'date':
                                            ...
                                            $daylist_start = $daylist_end = '<select name="CustomFields[' . $GLOBALS['FieldID'] . '][dd_whichone]" class="datefield form-control sendDate">';
                                            for ($i = 1; $i <= 31; $i++) {
                                                $dom = $i;
                                                $i = sprintf("%02d", $i);
                                                $sel = '';
                                            ...
                                            $monthlist_start = $monthlist_end = '<select name="CustomFields[' . $GLOBALS['FieldID'] . '][mm_whichone]" class="datefield form-control sendDate">';
                                            for ($i = 1; $i <= 12; $i++) {
                                                $mth = $i;
                                                $sel = '';
                                                $i = sprintf("%02d", $i);
                                            ...
                                            $yearlist_start = '<input type="text" maxlength="4" size="4" value="' . $yy_start . '" name="CustomFields[' . $GLOBALS['FieldID'] . '][yy_whichone]" class="datefield form-control" style="padding-left:5px;">';
                                            $yearlist_end = '<input type="text" maxlength="4" size="4" value="' . $yy_end . '" name="CustomFields[' . $GLOBALS['FieldID'] . '][yy_whichone]" class="datefield form-control" style="padding-left:5px;">';
                                        ...
                                        case 'checkbox':
                                            ...
                                            foreach ($fieldsettings['Key'] as $pos => $key) {
                                                ...
                                                $optionlist .= '<label class="customCheckBox" for="' . $label_id . '"><input type="checkbox" name="' . $label_id . '" id="' . $label_id . '" value="1"' . $checked . '>' . htmlspecialchars($fieldsettings['Value'][$pos], ENT_QUOTES, SENDSTUDIO_CHARSET) . '<span class="checkmark"></span>';
                                                ...
                                                $span = '&nbsp;<span class="' . $GLOBALS['SpanID'] . '_and" style="display:' . $and_display . ';">' . GetLang('AND') . '</span>&nbsp;<span class="' . $GLOBALS['SpanID'] . '_or" style="display:' .
                                                        $or_display . ';">' . GetLang('OR') . '&nbsp;</span> </label>';
                                                $optionlist .= $span;
    ...
    function PrintSubscribeDate() {
        ...
        $daylist_start = $daylist_end = '<select name="datesearch[dd_whichone]" class="datefield sendDate form-control">';
        for ($i = 1; $i <= 31; $i++) {
            $dom = $i;
            $i = sprintf("%02d", $i);
            $sel = '';
        ...
        $monthlist_start = $monthlist_end = '<select name="datesearch[mm_whichone]" class="datefield sendDate form-control">';
        for ($i = 1; $i <= 12; $i++) {
            $mth = $i;
            $sel = '';
            $i = sprintf("%02d", $i);
        ...
        $yearlist_start = '<input type="text" maxlength="4" size="4" value="' . $yy_start . '" name="datesearch[yy_whichone]" class="datefield form-control" style="padding-left:5px;">';
        $yearlist_end = '<input type="text" maxlength="4" size="4" value="' . $yy_end . '" name="datesearch[yy_whichone]" class="datefield form-control" style="padding-left:5px;">';
