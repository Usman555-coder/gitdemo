...
class Subscribers_Import extends Subscribers
{
	...
	function LinkFields()
	{
		...
		foreach ($customfields as $pos => $details) {
			...
			if ($details['fieldtype'] == 'date') {
				...
				switch ($temp_name) {
					case 'mm/dd/yyyy':
                        $details['fieldid'] .= '|SD_MDY';
                    break;

					case 'dd/mm/yyyy':
                        $details['fieldid'] .= '|SD_DMY';
                    break;

					case 'yyyy/mm/dd':
                        $details['fieldid'] .= '|SD_YMD';
                    break;
				}
			}
					...
					if(count($arr_type) == 1)
						...
						switch($arr_type[1]){
							case 'SD_MDY':
								if (substr_count($subscriberinfo[$pos], '/') == 2) {
									$found_subscribedate = true;
									list($c_m, $c_d, $c_y) = explode('/', $subscriberinfo[$pos]);
								}
							break;
							...
							case 'SD_DMY':
								if (substr_count($subscriberinfo[$pos], '/') == 2) {
									$found_subscribedate = true;
									list($c_d, $c_m, $c_y) = explode('/', $subscriberinfo[$pos]);
								}
							break;
							...
							case 'SD_YMD':
								if (substr_count($subscriberinfo[$pos], '/') == 2) {
									$found_subscribedate = true;
									list($c_y, $c_m, $c_d) = explode('/', $subscriberinfo[$pos]);
								}
							break;
						}
			...
			*/
			...
			if ($fieldtype == 'date') {
				...
				if ($fielddata['yyyy'] >= 0 && $fielddata['mm'] >= 0 && $fielddata['dd'] >= 0) {
					...
					if ($valid_subscribedate) {
						$curr_custom_date = array('dd' => $fielddata['dd'], 'mm' => $fielddata['mm'], 'yy' => $fielddata['yyyy']);
						
						//$importresults['baddata'][] = implode($importinfo['FieldSeparator'], $curr_custom_date);
						//$curr_custom_date = AdjustTime(array(0, 0, 1, $fielddata['mm'], $fielddata['dd'], $fielddata['yyyy']), true);
						//$curr_custom_date = date('d/m/Y', $curr_custom_date);
					} else {
						$subscriberinfo[] = GetLang('InvalidSubscribeDate');
						$importresults['bads']++;
						$importresults['baddata'][] = implode($importinfo['FieldSeparator'], $subscriberinfo);
				...
				$fielddata = $subf->CheckData($curr_custom_date, true);
				$customfields[$fieldid] = $fielddata;
			}
		}
	...
	*/
	function _GenerateMapTip($fieldname=false)
	{
		...
		$helptip = '<img onMouseOut="HideHelp(\'' . $rand . '\');" onMouseOver="ShowHelp(\'' . $rand . '\', \'' . $tiptitle . '\', \'' . $tipname . '\');" src="assets/images/infoIconBlue.svg" style="display: inline-block;margin: 10px 0px;" width="24" height="16" border="0"><div style="display:none" class="helptoolTipNew" id="' . $rand . '"></div>';
		return $helptip;
	}
