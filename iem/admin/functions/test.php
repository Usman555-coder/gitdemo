 ...
 * @subpackage SendStudio_Functions
 */
class test extends SendStudio_Functions {
    ...
    function Process() {
        ...
        if ($action == 'remove'){ $permission_action = 'delete'; }
        if ($action == 'view')  { $permission_action = 'manage'; }
        ...
        //var_dump($subaction); exit;

        if (!$access) { $this->DenyAccess(); return; }
...
//        $ManageSubscribers->Process($subaction);
    ...
    function ChooseList($action = 'Manage', $subaction = null, $autoredirect = true) {
        ...
        if (sizeof($listids) < 1) {
            ...
            //var_dump($this->ParseTemplate('Subscribers_No_Lists')); exit;
